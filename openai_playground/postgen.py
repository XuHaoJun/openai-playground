...
"start with # and space between each hashtag"
  (few-shots 
...
(defun 產生行銷文宣 (平台 [寫作風格 List] [回覆設定 List] user-input)
  "目的是讓人們點擊鏈接進行購買，或者是行銷自己"
  (let* ((Response (-> 
      (use-role system-role)
      (電子商務專家 平台)
      (在地化專家 (地點 . 台灣))
      (刺激消費 (關鍵提煉 (通俗易懂 (社群媒體專家 寫作風格))))
      user-input)))
    (text-format (回覆設定 Response))
  )
)
...
(產生行銷文宣 
  ...
  (list (num-hash-tag 3) (字數 "20~30字") (語言 "繁體中文")))
  "國慶日活動,商品看圖片,多使用圖片內容"
)
  """
  return prompt
...
def call_llm(prompt):
  messages = [
    ...
    {'content': [{'type': 'text', 'text': create_user_prompt()},
     {'type': 'image_url', 'image_url': {'url':'https://img1.momoshop.com.tw/expertimg/0007/415/256/mobile/20240724-03.jpg?t=1725869552369'}}
     ], 'role': 'user'}
  ]
